...
import pygame
# Import the start_level1 function from level_1.py
from level_1 import start_level1


# Window dimensions - playground
WIDTH = 420
HEIGHT = 250
...
# Create the main window (where we'll play)
...
# Create buttons - 6 in total (to choose a level)
buttons = []
for i in range(1, 7):
...
while running:
      ...
      if event.button == 1:
        ...
        for i, button in enumerate(buttons):
          if button.collidepoint(event.pos):
            # Since Python indexes start from 0 (level 1 will be 0 for python ,etc.)
            level = i + 1
  ...
  # update the content on the window 
  pygame.display.flip()
